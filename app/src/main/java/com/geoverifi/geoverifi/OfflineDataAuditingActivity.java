...
import com.geoverifi.geoverifi.adapter.OfflineSubmissionsAuditingAdapter;
...
public class OfflineDataAuditingActivity extends AppCompatActivity {
    Context context = this;
    RecyclerView offlineRecyclerView;
    List<SubmissionAuditing> submissionList = new ArrayList<SubmissionAuditing>();
    OfflineSubmissionsAuditingAdapter offlineSubmissionsAuditingAdapter;
    int upload = 0;
    DatabaseHandler db;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_offline_auditingdata);
        ...
        submissionList = db.allOfflineSubmissionsauditing();
        updateUI();


        offlineSubmissionsAuditingAdapter = new OfflineSubmissionsAuditingAdapter(context, submissionList);
        offlineRecyclerView.setAdapter(offlineSubmissionsAuditingAdapter);

        getSupportActionBar().setTitle(getResources().getString(R.string.waitings));
...
//                break;
    ...
    private void submitOfflineData(final SubmissionAuditing s, final int index){
        String url = Variables.BASE_URL + "API/addAuditing";
        ...
        params.put("quality", createPartFromString(s.get_trafficspeed()));
        params.put("angle", createPartFromString(s.get_trafficquantity()));
        ...
        params.put("traffic_quantity", createPartFromString(String.valueOf(s.get_trafficquantity())));
        params.put("traffic_speed", createPartFromString(String.valueOf(s.get_trafficspeed())));
        ...
        call.enqueue(new Callback<com.geoverifi.geoverifi.server.model.Response>() {
            ...
            public void onResponse(Call<com.geoverifi.geoverifi.server.model.Response> call, retrofit2.Response<com.geoverifi.geoverifi.server.model.Response> response) {
                if (response.body().isStatus()) {
                    db.deleteOfflineSubmissionAuditing(s.get_id());
                    submissionList.remove(index);
                    offlineRecyclerView.removeViewAt(index);
                    offlineSubmissionsAuditingAdapter.notifyItemRemoved(index);
                    offlineSubmissionsAuditingAdapter.notifyItemRangeChanged(index, submissionList.size());
                    offlineSubmissionsAuditingAdapter.notifyDataSetChanged();
                }else{
                    Toast.makeText(OfflineDataAuditingActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<com.geoverifi.geoverifi.server.model.Response> call, Throwable t) {
                Toast.makeText(OfflineDataAuditingActivity.this, t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
